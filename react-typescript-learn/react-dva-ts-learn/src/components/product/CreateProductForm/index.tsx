...
const CreateProductForm: FunctionComponent<CreateProductFormProps> = ({ form }) => {
  const uploadImageEvent = (event: any) => {
    ...
    <div>
      <Form layout="horizontal" onSubmit={handleSubmit}>
        ...
        <Form.Item label="商品库存" {...formItemLayout}>
          ...
          })(<InputNumber placeholder="请输入商品库存" name="productStock" min={0} max={10000} step={1} />)}
        </Form.Item>
        <Form.Item label="商品价格" {...formItemLayout}>
          {getFieldDecorator('productPrice', {
            rules: [{ required: true, message: '请输入商品价格!' }]
          })(<InputNumber placeholder="请输入商品价格" name="productPrice" min={0} max={10000} step={1} />)}
        </Form.Item>
        ...
        <Form.Item label="商品图片" {...formItemLayout}>
          {getFieldDecorator('productImage', {
            ...
            getValueFromEvent: uploadImageEvent
          })(
