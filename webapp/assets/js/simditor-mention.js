...
SimditorMention = (function(superClass) {
  ...
  SimditorMention.prototype.getItems = function(filter) {
    return $.ajax({
      type: 'get',
      url: this.opts.mention.url+"?name="+filter
    }).done((function(_this) {
      return function(result) {
        _this.items = JSON.parse(result);
  ...
  SimditorMention.prototype.refresh = function() {
	...
	s = targetOffset.left - wrapperOffset.left + this.target.width()
	console.log("popoverEl:",targetOffset.left - wrapperOffset.left + this.target.width());
  ...
  SimditorMention.prototype._renderPopover = function() {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      item = ref[j];
      name = item[this.opts.mention.nameKey] +" - "+item[this.opts.mention.staffId]   ;
  ...
  SimditorMention.prototype.selectItem = function() {
    ...
    $itemLink = $('<a/>', {
      ...
      text: '@' + $selectedItem.attr('data-name').split(" - ")[0],
      href: href,
      'data-mention': true
    });
  ...
  SimditorMention.prototype.filterItem = function() {
	...
	//console.log("filter:",val);
	   ...
	   async:false,
      type: 'get',
      url: this.opts.mention.url+"?name="+ val
    }).done((function(_this) {
      return function(result) {
        _this.items = JSON.parse(result);
	//	console.log("search",_this.items)
